...
# Control reasoning/thinking effort (low, medium, high)
git-commit-ai generate --thinking-effort low
...
- `GIT_COMMIT_AI_THINKING_EFFORT` (optional): Reasoning effort level (`low`, `medium`, `high`). Controls how much thinking the model performs. Use `low` for faster responses or when using non-reasoning models.
...
Use the `--debug` flag to see detailed output including token usage and reasoning:
...
- Git diff preview (first 500 characters)
- Model being used
- **Token Usage**: input tokens, output tokens, total tokens
- **Reasoning Tokens**: number of tokens used for thinking (if supported by model)
- **Cache Tokens**: cache read/write tokens (if available)
- **Reasoning Output**: the full thinking/reasoning text generated by the model
...
🔍 Debug Information:
  Token Usage:
    ...
    Reasoning tokens: 45
  Reasoning Output:
    The user added a new authentication module...
    I should generate a commit message about...
```
